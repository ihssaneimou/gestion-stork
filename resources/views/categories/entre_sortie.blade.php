<x-nav-bar>
    ...
    <div class="fixed font-mon bg-white grid hidden rounded-md shadow-md  deleteGroupModal  z-50" id="deleteGroupModal"
        ...
        <div class="grid justify-items-center w-full">
            <form id="deleteForm" method="POST" class="p-6 w-full">
                @csrf
                @method('delete')
                ...
                <div class="mt-6 flex justify-end">
                    ...
                    <button type="submit"
                        class=' ms-3 inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 active:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150'id="sub"></button>
                </div>
            </form>
        </div>
    </div>
    <div class="text-sm font-medium text-center text-gray-500  border-gray-200 dark:text-gray-400 dark:border-gray-700"
        ...
        <div class="flex justify-between border-b-2  ">
            @if (isset($categories))
                ...
                <form action="{{route('categories.entre_sortie', $categories)}}" method="GET" class="relative w-2/5">
            ...
            @else
                ...
                <form action="{{route('categories_Autre.entre_sortie')}}" method="GET" class="relative w-2/5">
            ...
            <tbody>
                @foreach ($tous as $item)
                    <tr class="bg-white  hover:bg-gray-50" href='categories.index_mar_'>
                        <td class="py-4 px-2 sm:px-6">{{ $item->type }}</td>
                        <td class="py-4 px-2 sm:px-6">{{ $item->quantite }}</td>
                        <td class="py-4 px-2 sm:px-6">{{ $item->created_at }}</td>
