                ...
                <script>
                    $(document).ready(function() {
                        $('#type').change(function() {
                            ...
                            var url = source === 'index_adm' ? "{{ route('adm_t', ['user' => ':user']) }}".replace(':user', user) : "{{ route('type') }}";
                            $.ajax({
                                url: url,
                                type: 'GET',
                                data: {
                                ...
                                success: function(response) {
                                    ...
                                    response.activites.forEach(function(activity) {
                                        ...
                                        var user =  source === 'index_adm' ?  '{{ $activites[0]->user->name }}':response.users.find(user => user.id === activity.id_adm) ;
